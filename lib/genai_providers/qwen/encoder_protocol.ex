defprotocol GenAI.Provider.Qwen.EncoderProtocol do
  @moduledoc """
...
# GenAI.Tool
# -----------------------------
defimpl GenAI.Provider.Qwen.EncoderProtocol, for: GenAI.Tool do
...
# GenAI.Message
# -----------------------------
defimpl GenAI.Provider.Qwen.EncoderProtocol, for: GenAI.Message do
  ...
  def content(%GenAI.Message.Content.ThinkingContent{} = content) do
    content.thinking
  end
  ...
  defp image_part(%GenAI.Message.Content.ImageContent{} = content) do
    {:ok, encoded} = GenAI.Message.Content.ImageContent.base64(content)
    base64 = "data:image/#{content.type};base64," <> encoded
    %{type: :image_url, image_url: %{url: base64}}
  end

  defp content_part(content) when is_bitstring(content), do: %{type: :text, text: content}
  defp content_part(%GenAI.Message.Content.TextContent{} = c), do: %{type: :text, text: c.text}
  defp content_part(%GenAI.Message.Content.ImageContent{} = content), do: image_part(content)

  defp content_part(%GenAI.Message.Content.AudioContent{} = content) do
    case content.transcript do
      t when is_bitstring(t) and t != "" -> %{type: :text, text: t}
      _ -> nil
    end
  end
  ...
  defp image_part?(%GenAI.Message.Content.ImageContent{}), do: true
    ...
    encoded =
      case subject.content do
        ...
        x when is_list(x) ->
          ...
          content =
            if Enum.any?(rest, &image_part?/1) do
              rest
...
# -----------------------------
defimpl GenAI.Provider.Qwen.EncoderProtocol, for: GenAI.Message.ToolResponse do
  def encode(subject, _model, session, _context, _options) do
    encoded = %{
      role: :tool,
...
# -----------------------------
defimpl GenAI.Provider.Qwen.EncoderProtocol, for: GenAI.Message.ToolUsage do
  def content(content)
  ...
  def encode(subject, _model, session, _context, _options) do
    content =
      cond do
        ...
        is_nil(subject.content) -> []
        true -> [subject.content]
      end
